...
- **Edge** — a directed connection from one vertex (source) to another (destination).
- **Adjacency List** — a way to represent the graph where each vertex stores a list of its neighbors.
...
#### Why Adjacency List?
An adjacency list is memory-efficient for sparse graphs (graphs with fewer edges). It stores only existing connections, unlike an adjacency matrix which always uses O(V²) space.
...
- As graph size increases from 10 → 30 → 100 vertices, execution time grows proportionally, confirming the expected linear complexity.
